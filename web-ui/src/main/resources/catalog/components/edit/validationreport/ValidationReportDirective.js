...
(function () {
   ...
   */
  angular
    .module("gn_validation_report_directive", ["gn_utility"])
          ...
          link: function (scope, element, attrs) {
              ...
              gnValidation
                ...
                .then(function (response) {
                  ...
                  angular.forEach(ruleTypes, function (ruleType) {
                    ...
                    scope.hasRequiredErrors =
                      scope.hasRequiredErrors ||
                      (ruleType.requirement === "REQUIRED" && ruleType.error > 0);
                    scope.hasErrors = scope.hasErrors || ruleType.error > 0;
                    angular.forEach(ruleType.patterns, function (pat) {
                      scope.numberOfRules += pat.rules ? pat.rules.length : 0;
                    });
                  });
            ...
            scope.getClass = function (type) {
              if (scope.numberOfRules > 0) {
                if (type === "icon") {
                  return scope.hasRequiredErrors ? "fa-thumbs-o-down" : "fa-thumbs-o-up";
                }
