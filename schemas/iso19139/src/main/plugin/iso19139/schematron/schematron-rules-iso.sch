  ...
  <sch:ns prefix="gml320" uri="http://www.opengis.net/gml"/>
	...
	<sch:pattern>
		...
		<sch:rule context="//gmd:MD_LegalConstraints[gmd:accessConstraints/gmd:MD_RestrictionCode/@codeListValue='otherRestrictions']
			...
			<sch:let name="access" value="not(gmd:otherConstraints)
				or count(gmd:otherConstraints[(gco:CharacterString and gco:CharacterString = '' and not(@gco:nilReason='missing'))
				or (gmx:Anchor and gmx:Anchor = '' and not(@gco:nilReason='missing'))]) > 0"/>
			<sch:assert
				test="$access = false()"
				>
		...
		<sch:rule context="//gmd:MD_LegalConstraints[gmd:useConstraints/gmd:MD_RestrictionCode/@codeListValue='otherRestrictions']
			|//*[@gco:isoType='gmd:MD_LegalConstraints' and gmd:accessConstraints/gmd:MD_RestrictionCode/@codeListValue='otherRestrictions']">
			<sch:let name="use" value="not(gmd:otherConstraints)
				or count(gmd:otherConstraints[(gco:CharacterString and gco:CharacterString = '' and not(@gco:nilReason='missing')) or (gmx:Anchor and gmx:Anchor = '' and not(@gco:nilReason='missing'))]) > 0"/>
			<sch:assert
				test="$use = false()"
				>
    ...
    <sch:pattern>
        ...
        <sch:rule context="/*/gmd:dateStamp">
          ...
          <sch:report test="$date">
            <sch:value-of select="concat($loc/strings/report.requiredDateStamp, ': ', $dateStamp)"/>
          </sch:report>
        </sch:rule>
        ...
        <sch:rule context="//gmd:identificationInfo/*/gmd:language">
            <sch:assert test="node() | @gco:nilReason">
                <sch:value-of select="$loc/strings/alert.requiredIdentificationLanguage"/>
            </sch:assert>
            <sch:report test="node() | @gco:nilReason" >
                <sch:value-of select="$loc/strings/report.requiredIdentificationLanguage"/>
            </sch:report>
        </sch:rule>
        <sch:rule context="//gmd:MD_Metadata/gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:report">
            <sch:let name="countConformanceResult"
                     value="count(*/gmd:result/gmd:DQ_ConformanceResult)"/>
            <sch:let name="countQuantitativeResult"
                     value="count(*/gmd:result/gmd:DQ_QuantitativeResult)"/>
            <sch:assert test="$countConformanceResult &lt; 2 ">
              <sch:value-of select="concat($loc/strings/alert.countConformanceResult,' (Check the results in report ',position()-1,')')"/>
            </sch:assert>
            <sch:assert test="$countQuantitativeResult &lt; 2">
              <sch:value-of select="concat($loc/strings/alert.countQuantitativeResult,' (Check the results in report ',position()-1,')')"/>
            </sch:assert>
            <sch:assert test="($countConformanceResult + $countQuantitativeResult) > 0">
              <sch:value-of select="concat($loc/strings/alert.minimumOneConformanceResultOrQuantitativeResult,' (Check the results in report ',position()-1,')')"/>
            </sch:assert>
            <sch:report test="($countConformanceResult = 1 or $countQuantitativeResult = 1) and not($countConformanceResult > 1 or $countQuantitativeResult >1)">
              <sch:value-of select="concat($loc/strings/report.minimumOneConformanceResultOrQuantitativeResult,' (Report ',position()-1,')')"/>
            </sch:report>
        </sch:rule>
    </sch:pattern>
